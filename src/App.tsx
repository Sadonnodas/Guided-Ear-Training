...
import { getScaleStepsFromRoot, getDegreeLabelFromStep } from "./audio/MusicTheory";
...
const StopIcon = () => <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="1"/></svg>;
...
// VISUALIZER CONFIG
const CELL_WIDTH = 60; // Must match .tape-cell width in CSS
const TAPE_RANGE = 24; // Static Grid Range
...
export default function App() {
  ...
  // Track specific MIDI for visualizer accuracy
  const [activeMidi, setActiveMidi] = useState<number | null>(null);
  const [lastValidStep, setLastValidStep] = useState(0); // Prevents twitching on silence
  ...
  // Removed setVolClick destructuring to clean warning
  ...
  useEffect(() => {
    audioEngine.onNotePlay = (note, isClick) => {
      ...
      if (note) {
        ...
        const secPerBeat = 60 / bpm; // Fixed typo here (was constQX)
  ...
  // STABILIZER LOGIC:
  // When activeMidi updates, if it is NOT null, update the tape position.
  // If it IS null (silence), do nothing (leave tape where it was).
  useEffect(() => {
    if (activeMidi !== null) {
  ...
  const stopSession = () => {
    ...
    setStatus("Paused");
    setActiveMidi(null);
    setLastValidStep(0); // Return to center on stop
  };
  ...
  // --- TAPE VISUALIZER LOGIC ---
  const renderTape = () => {
    // FIX: Use lastValidStep instead of calculating from activeMidi directly.
    const currentSteps = lastValidStep;
    ...
    const dynamicOffset = currentSteps * CELL_WIDTH;
    const totalTranslate = -(initialOffset + dynamicOffset); // Fixed typo here (was constHV)
    ...
    return (
        <div className="tape-strip" style={{ transform: `translateX(${totalTranslate}px)` }}>
            {staticCells.map((stepIndex) => {
                ...
                // Active only if it matches current scale step AND we are actually playing a note
                const isActive = stepIndex === currentSteps && activeMidi !== null;
                const isEnabled = enabledDegrees.includes(label as ScaleDegree);
                
                // Assign specific color class
                const degreeClass = `d-${label}`;

                return (
                    <div 
                        key={stepIndex} 
                        className={`tape-cell ${degreeClass} ${isActive ? 'active' : ''} ${isEnabled ? '' : 'disabled'}`}
      ...
      <div className="main-panel">
        ...
        <div className={`controls-accordion ${settingsOpen ? 'open' : ''}`}>
          <div className="controls-content">
            ...
            {/* Click slider is hidden in UI but kept in state logic if needed later */}
            <div className="slider-row">
